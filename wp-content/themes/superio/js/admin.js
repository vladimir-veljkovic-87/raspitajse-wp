jQuery(document).ready(function($){
    ...
    function show_hide_page_setting(val) {
        ...
        // Candidates template
        else if ( val == 'page-candidates.php' ) {
    ...
    $(document).on('change', '#apus_page_layout_type', function(){
        var val = $(this).val();
        if ( val == 'default' || val == 'top-map' || val == '' ) {
            $('.cmb2-id-apus-page-jobs-show-filter-top').show();
            $('.cmb2-id-apus-page-jobs-filter-top-sidebar').show();
        } else {
            $('.cmb2-id-apus-page-jobs-show-filter-top').hide();
            $('.cmb2-id-apus-page-jobs-filter-top-sidebar').hide();
        }
    });
    setTimeout(function() {
    ...
    $(document).on('change', '#apus_page_jobs_show_filter_top', function(){
        var val = $(this).val();
        if ( val == 'no' ) {
            $('.cmb2-id-apus-page-jobs-filter-top-sidebar').hide();
        } else {
    ...
    setTimeout(function() {
        if ( $('#apus_page_jobs_show_filter_top').length > 0 ) {
            var val = $('#apus_page_jobs_show_filter_top').val();
            if ( val == 'no' ) {
                $('.cmb2-id-apus-page-jobs-filter-top-sidebar').hide();
            } else {
                $('.cmb2-id-apus-page-jobs-filter-top-sidebar').show();
    ...
    // employer
    $(document).on('change', '#apus_page_employers_display_mode', function(){
        var val = $(this).val();
        show_hide_employer_setting(val);
    });
    setTimeout(function() {
        if ( $('#apus_page_employers_display_mode').length > 0 ) {
            var val = $('#apus_page_employers_display_mode').val();
            show_hide_employer_setting(val);
        }
    }, 100);
    function show_hide_employer_setting(val) {
        if ( val == 'grid' ) {
            $('.cmb2-id-apus-page-employers-inner-grid-style').show();
            $('.cmb2-id-apus-page-employers-inner-list-style').hide();
        } else if( val == 'list' ) {
            $('.cmb2-id-apus-page-employers-inner-grid-style').hide();
            $('.cmb2-id-apus-page-employers-inner-list-style').show();
        }
    }

    // candidate
    $(document).on('change', '#apus_page_candidates_display_mode', function(){
        var val = $(this).val();
        show_hide_candidate_setting(val);
    });
    setTimeout(function() {
        if ( $('#apus_page_candidates_display_mode').length > 0 ) {
            var val = $('#apus_page_candidates_display_mode').val();
            show_hide_candidate_setting(val);
        }
    }, 100);
    function show_hide_candidate_setting(val) {
        if ( val == 'grid' ) {
            $('.cmb2-id-apus-page-candidates-inner-grid-style').show();
            $('.cmb2-id-apus-page-candidates-inner-list-style').hide();
        } else if( val == 'list' ) {
            $('.cmb2-id-apus-page-candidates-inner-grid-style').hide();
            $('.cmb2-id-apus-page-candidates-inner-list-style').show();
        }
    }

    $(document).on('change', '#apus_page_candidates_layout_type', function(){
        var val = $(this).val();
        if ( val == 'default' || val == 'top-map' || val == '' ) {
            $('.cmb2-id-apus-page-candidates-show-filter-top').show();
            $('.cmb2-id-apus-page-candidates-filter-top-sidebar').show();
        } else {
            $('.cmb2-id-apus-page-candidates-show-filter-top').hide();
            $('.cmb2-id-apus-page-candidates-filter-top-sidebar').hide();
        }
    });
    setTimeout(function() {
        if ( $('#apus_page_candidates_layout_type').length > 0 ) {
            var val = $('#apus_page_candidates_layout_type').val();
            if ( val == 'default' || val == 'top-map' || val == '' ) {
                $('.cmb2-id-apus-page-candidates-show-filter-top').show();
                $('.cmb2-id-apus-page-candidates-filter-top-sidebar').show();
            } else {
    ...
    $(document).on('change', '#apus_page_candidates_show_filter_top', function(){
        var val = $(this).val();
        if ( val == 'no' ) {
            $('.cmb2-id-apus-page-candidates-filter-top-sidebar').hide();
        } else {
    ...
    setTimeout(function() {
        if ( $('#apus_page_candidates_show_filter_top').length > 0 ) {
            var val = $('#apus_page_candidates_show_filter_top').val();
            if ( val == 'no' ) {
                $('.cmb2-id-apus-page-candidates-filter-top-sidebar').hide();
            } else {
                $('.cmb2-id-apus-page-candidates-filter-top-sidebar').show();
    ...
    // loading admin icon
    $("body").on("wp_job_board_pro_all_loading_icons", function(){
        superio_get_awesome_icons();
        superio_get_flaticon_icons();
        superio_get_themify_icons();
    });

    function superio_get_awesome_icons() {
        ...
        wp_job_board_pro_all_loaded_icons.fontwesome = icons_load_call;
    }

    function superio_get_flaticon_icons() {
        ...
        for( var i=0; i < files.length; i++ ){
            css = Array.prototype.map.call( files[i].sheet.cssRules, superio_css_text ).join('\n');
        ...
        wp_job_board_pro_all_loaded_icons.flaticon = icons_load_call;
    }

    function superio_get_themify_icons() {
        ...
        wp_job_board_pro_all_loaded_icons.themify = icons_load_call;
    }

    function superio_css_text(x) {
        return x.cssText;
    }
});