...
$download_urls = [];
if ( $cv_file_id ) {
    if ( is_array($cv_file_id) ) {
        foreach ($cv_file_id as $id) {
            $download_urls[] = add_query_arg(array('file_id' => $id), $download_base_url);
        }
    } else {
        $download_urls[] = add_query_arg(array('file_id' => $cv_file_id), $download_base_url);
    }
...
if ( empty($download_urls) ) {
    ...
    $download_urls[] = add_query_arg(array('post_id' => $candidate_id), $download_base_url);
}
...
<article <?php post_class('applicants-job job-applicant-wrapper '.$classes); ?>>
    <div class="candidate-list candidate-archive-layout flex-middle-sm">
        ...
        <div class="ali-right">
            <div class="applicant-action-button action-button">
                ...
                <a data-toggle="tooltip" href="javascript:void(0);" class="btn-approve-job-applied btn-action-icon approve" data-applicant_id="<?php echo esc_attr($post->ID); ?>" data-nonce="<?php echo esc_attr(wp_create_nonce( 'wp-job-board-pro-approve-applied-nonce' )); ?>" title="<?php echo esc_attr_e('Approve Application', 'superio'); ?>"><i class="ti-check"></i></a>

                <a data-toggle="tooltip" title="<?php echo esc_attr_e('Reject Application', 'superio'); ?>" href="javascript:void(0);" class="btn-action-icon btn-reject-job-applied rejec" data-applicant_id="<?php echo esc_attr($post->ID); ?>" data-nonce="<?php echo esc_attr(wp_create_nonce( 'wp-job-board-pro-reject-applied-nonce' )); ?>"><i class="ti-loop"></i></a>
