...
<table class="head container" style="border-bottom:1px dotted #1967d2; margin-bottom:20px;">
	<tr>
		<td class="shop-info">
		...
		<td class="header" style="padding-bottom:10px;text-align:right;">
...
<?php if ( $this->has_header_logo() ) : ?>
	<h1 class="document-type-label">FAKTURA</h1>
<?php endif; ?>
...
<table class="order-data-addresses">
	<tr>
		<td class="address billing-address">
			...
			<strong style="display:block;margin-bottom:2px;">Kupac:</strong>
			<p><?php $this->billing_address(); ?></p>
			<?php
			// Kupac: Matični broj + PIB (iz order meta)
			$mb  = $this->order->get_meta('_billing_mb');
			$pib = $this->order->get_meta('_billing_pib');

			if ( !empty($mb) || !empty($pib) ) : ?>
				<div class="customer-company-data" style="margin-top:6px;font-size:11px;line-height:1;">
					<?php if ( !empty($mb) ) : ?>
						<div><strong>Matični broj:</strong> <?php echo esc_html($mb); ?></div>
					<?php endif; ?>

					<?php if ( !empty($pib) ) : ?>
						<div><strong>PIB:</strong> <?php echo esc_html($pib); ?></div>
					<?php endif; ?>
				</div>
			<?php endif; ?>		
...
<div class="invoice-legal-note" style="margin-top:20px;font-size:11px;line-height:1.3;color:#555;">
	Napomena o poreskom oslobođenju:<br>
	<strong>Paušalno oporezovani preduzetnik</strong><br><br>

	Faktura je validna bez pečata i potpisa.
</div>
