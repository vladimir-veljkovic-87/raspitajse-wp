...
class WP_Job_Board_Pro_Post_Type_Job_Alert {
	...
	public static function register_post_type() {
		$singular = __( 'Job Alert', 'wp-job-board-pro' );
		$plural   = __( 'Job Alerts', 'wp-job-board-pro' );
		...
		if ( isset($_GET['post']) && $_GET['post'] && is_admin() ) {
			...
			if ( $post && $post->post_type == 'job_alert' ) {
				...
				$fields[] = array(
					...
					'type' => 'title',
					'id'   => WP_JOB_BOARD_PRO_JOB_ALERT_PREFIX . 'author'
				);
			}
		}
		$fields[] = array(
			'name'              => __( 'Alert Query', 'wp-job-board-pro' ),
			'id'                => WP_JOB_BOARD_PRO_JOB_ALERT_PREFIX . 'alert_query',
			'type'              => 'textarea',
		);
		$fields[] = array(
			'name'              => __( 'Email Frequency', 'wp-job-board-pro' ),
			'id'                => WP_JOB_BOARD_PRO_JOB_ALERT_PREFIX . 'email_frequency',
			'type'              => 'select',
			'options'			=> $email_frequency
		);
		$metaboxes[ WP_JOB_BOARD_PRO_JOB_ALERT_PREFIX . 'general' ] = array(
			'id'                        => WP_JOB_BOARD_PRO_JOB_ALERT_PREFIX . 'general',
	...
	public static function custom_columns($columns) {
		...
		$fields = array_merge($columns, array(
			...
			'author' 			=> esc_html__( 'Author', 'wp-job-board-pro' ),
		));
		return $fields;
	}
	...
	public static function custom_columns_manage( $column ) {
		switch ( $column ) {
			case 'email_frequency':
					$email_frequency = get_post_meta( get_the_ID(), WP_JOB_BOARD_PRO_JOB_ALERT_PREFIX . 'email_frequency', true );
...
WP_Job_Board_Pro_Post_Type_Job_Alert::init();