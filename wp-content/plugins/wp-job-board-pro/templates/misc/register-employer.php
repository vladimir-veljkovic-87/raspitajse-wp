		...
		<?php endif; ?>
  		...
  		<?php
			...
			$form_html = cmb2_get_metabox_form( $metaboxes_form, $post_id, array(
				'form_format' => '<form action="" class="cmb-form %1$s" method="post" id="%1$s_'.rand(0000,9999).'" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="'.$form_obj->get_form_name().'" value="'.$form_obj->get_form_name().'"><input type="hidden" name="object_id" value="%2$s">%3$s terms_and_conditions_html<input type="submit" name="submit-cmb-register-employer" value="%4$s" class="button-primary"></form>',
